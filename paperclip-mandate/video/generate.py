...
def scene_phishing_reveal():
    """12-18s: next morning post-mortem — you discover the address was a scam"""
    frames = []
    total = 6 * FPS
    header_lines = [
        ("Next morning. You check the address.", WHITE),
    ]
    scan_lines = [
    ...
    for i in range(total):
        ...
        # context line fades in first
        if progress < 0.25:
            a = min(1.0, progress / 0.15)
            center_text(draw, header_lines[0][0], y_base, f_ctx,
                         tuple(int(c * a) for c in GRAY))
        else:
            center_text(draw, header_lines[0][0], y_base, f_ctx, GRAY)

        # scan results appear after context
        if progress > 0.2:
            scan_y = y_base + 80
            center_text(draw, "ADDRESS SCAN RESULTS", scan_y, f_head, RED_ALERT)

            scan_progress = (progress - 0.2) / 0.8
            lines_to_show = min(len(scan_lines), int(scan_progress * len(scan_lines) * 1.5) + 1)
            y = scan_y + 60
        ...
        # red flash pulses
        if 0.2 < progress < 0.3 or (0.4 < progress < 0.45):
            img = red_flash(img, 0.6)

        # glitch on UNRECOVERABLE reveal
        if 0.7 < progress < 0.88:
            img = apply_glitch(img, 12)

        if progress > 0.2:
            img = apply_shake(img, 6)
        frames.append(img)
    return frames
...
def _draw_slack_ui(draw, card_x, card_y, card_w, card_h):
    """Draw a simplified Slack-like chrome around the notification area."""
    # sidebar (dark purple, like Slack)
    sidebar_w = int(56 * SCALE)
    draw.rectangle([card_x, card_y, card_x + sidebar_w, card_y + card_h],
                     fill=(42, 16, 56))
    # workspace icon circle
    icon_r = int(16 * SCALE)
    icon_cx = card_x + sidebar_w // 2
    icon_cy = card_y + int(28 * SCALE)
    draw.ellipse([icon_cx - icon_r, icon_cy - icon_r,
                   icon_cx + icon_r, icon_cy + icon_r],
                  fill=(80, 40, 100))
    # "M" in the icon
    f_icon = font(16, bold=True)
    draw.text((icon_cx - int(6 * SCALE), icon_cy - int(8 * SCALE)),
               "M", font=f_icon, fill=WHITE)
    # sidebar dots (other channels)
    for j in range(3):
        dy = int((70 + j * 30) * SCALE)
        dot_r = int(4 * SCALE)
        draw.ellipse([icon_cx - dot_r, card_y + dy - dot_r,
                       icon_cx + dot_r, card_y + dy + dot_r],
                      fill=(80, 60, 90))
    ...
    draw.rectangle([hx, card_y, card_x + card_w, card_y + header_h],
                     fill=(30, 22, 38))
    f_chan = font(16, bold=True)
    draw.text((hx + int(16 * SCALE), card_y + int(12 * SCALE)),
               "# alerts-treasury", font=f_chan, fill=(200, 200, 210))
    # online dot
    dot_x = card_x + card_w - int(20 * SCALE)
    draw.ellipse([dot_x - 4, card_y + int(20 * SCALE) - 4,
                   dot_x + 4, card_y + int(20 * SCALE) + 4],
                  fill=GREEN_CALM)

    return sidebar_w, header_h


def scene_notification():
    """44-50s: Slack-like notification with bot message"""
    frames = []
    total = 6 * FPS
    ...
    for i in range(total):
        ...
        # Slack chrome
        sidebar_w, header_h = _draw_slack_ui(draw, card_x, card_y, card_w, card_h)

        # message area
        msg_x = card_x + sidebar_w + int(16 * SCALE)
        msg_y = card_y + header_h + int(16 * SCALE)
        ...
        if slide > 0.3:
            ...
            draw.ellipse([av_cx - av_r, av_cy - av_r,
                          ...
                          fill=MANDATE_BLUE)
            # shield icon in avatar
            f_av = font(18, bold=True)
            draw.text((av_cx - int(7 * SCALE), av_cy - int(9 * SCALE)),
                       "M", font=f_av, fill=WHITE)

            # bot name + timestamp
            name_x = av_cx + av_r + int(10 * SCALE)
            draw.text((name_x, msg_y + int(2 * SCALE)),
                       "Mandate Bot", font=f_head, fill=WHITE)
            draw.text((name_x + int(120 * SCALE), msg_y + int(4 * SCALE)),
                       "APP  3:14 AM", font=f_sm, fill=GRAY)

            # message content
            content_lines = [
                ("Transaction BLOCKED", RED_ALERT, True),
                ("", WHITE, False),
                ...
                ("", WHITE, False),
                ("Funds: SAFE", GREEN_CALM, True),
                ("Circuit breaker: ACTIVATED", ORANGE, False),
            ]
            lines_shown = min(len(content_lines),
                               int(content_progress * len(content_lines) * 1.3) + 1)
            cy = msg_y + int(44 * SCALE)

            # left red accent bar (like Slack attachment)
            bar_top = cy
            for j in range(lines_shown):
                text, color, bold = content_lines[j]
                used_f = f_head if bold else f
                draw.text((msg_x + int(16 * SCALE), cy), text,
                           font=used_f, fill=color)
                cy += int(26 * SCALE)
            # red accent bar
            draw.rectangle([msg_x + int(4 * SCALE), bar_top,
                             msg_x + int(8 * SCALE), cy],
                            fill=RED_ALERT)

            # reactions at bottom (Slack-like)
            if content_progress > 0.8:
                ry = cy + int(12 * SCALE)
                reactions = [("eyes  3", (60, 50, 70)), ("rotating_light  1", (60, 50, 70))]
                rx = msg_x + int(16 * SCALE)
                for rtxt, rbg in reactions:
                    rw = int(len(rtxt) * 8 * SCALE) + int(16 * SCALE)
                    draw.rounded_rectangle([rx, ry, rx + rw, ry + int(24 * SCALE)],
                                            radius=int(4 * SCALE), fill=rbg,
                                            outline=(80, 70, 90))
                    draw.text((rx + int(6 * SCALE), ry + int(3 * SCALE)),
                               rtxt, font=f_sm, fill=(200, 200, 210))
                    rx += rw + int(8 * SCALE)
...
def scene_logo():
    ...
    for i in range(total):
        ...
        f_sub = font(22)
        center_text(draw, "Policy. Reputation. Simulation. Any wallet. Non-custodial.",
                     H // SCALE * 0.48, f_sub,
                     tuple(int(c * a) for c in GRAY))
