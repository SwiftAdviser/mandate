...
class AegisService
{
    ...
    /**
     * Screen a single address for risk (chain-agnostic).
     * Returns null on failure/degradation, or ['risk_level' => ...] on success.
     */
    public function screenAddress(string $address): ?array
    {
        if ($this->isCircuitOpen()) {
            return null;
        }

        // Only works for EVM addresses (0x prefix)
        ...
        try {
            $response = Http::timeout($this->timeout)
                ->retry($this->retries, 100)
                ->withHeaders($this->headers())
                ->get("{$this->apiUrl}/api/public/v1/extension/account/{$address}/toxic-score");
            ...
            $data = $response->json();
            $toxicScore = (float) ($data['toxic_score'] ?? $data['toxicScore'] ?? 0);

            $riskLevel = 'SAFE';
            if ($toxicScore > 50) {
                $riskLevel = 'CRITICAL';
            } elseif ($toxicScore >= 20) {
                $riskLevel = 'HIGH';
            }

            return ['risk_level' => $riskLevel, 'toxic_score' => $toxicScore];
        } catch (\Throwable $e) {
            Log::warning('[AegisService] Address screen failed', ['error' => $e->getMessage()]);
            $this->recordFailure();
