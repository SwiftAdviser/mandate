Title: ERC-8183: Agentic Commerce [DRAFT]

URL Source: https://eips.ethereum.org/EIPS/eip-8183
...
| Authors | Davide Crapis([@dcrapis](https://github.com/dcrapis)), Bryan Lim([@ai-virtual-b](https://github.com/ai-virtual-b)), Tay Weixiong([@twx-virtuals](https://github.com/twx-virtuals)), Chooi Zuhwa([@Zuhwa](https://github.com/Zuhwa)) |
| --- |
| Created | 2026-02-25 |
| Discussion Link | [https://ethereum-magicians.org/t/erc-8183-agentic-commerce/27902](https://ethereum-magicians.org/t/erc-8183-agentic-commerce/27902) |
| Requires | [EIP-20](https://eips.ethereum.org/EIPS/eip-20) |

Table of Contents
-----------------

*   [Abstract](https://eips.ethereum.org/EIPS/eip-8183#abstract)
*   [Motivation](https://eips.ethereum.org/EIPS/eip-8183#motivation)
*   [Specification](https://eips.ethereum.org/EIPS/eip-8183#specification)
    *   [State Machine](https://eips.ethereum.org/EIPS/eip-8183#state-machine)
    *   [Roles](https://eips.ethereum.org/EIPS/eip-8183#roles)
    *   [Job Data](https://eips.ethereum.org/EIPS/eip-8183#job-data)
    *   [Optional provider (set later)](https://eips.ethereum.org/EIPS/eip-8183#optional-provider-set-later)
    *   [Core Functions](https://eips.ethereum.org/EIPS/eip-8183#core-functions)
    *   [Attestation](https://eips.ethereum.org/EIPS/eip-8183#attestation)
    *   [Fees](https://eips.ethereum.org/EIPS/eip-8183#fees)
    *   [Hooks (OPTIONAL)](https://eips.ethereum.org/EIPS/eip-8183#hooks-optional)
    *   [Events](https://eips.ethereum.org/EIPS/eip-8183#events)

*   [Rationale](https://eips.ethereum.org/EIPS/eip-8183#rationale)
    *   [Extensions (OPTIONAL)](https://eips.ethereum.org/EIPS/eip-8183#extensions-optional)

*   [Backwards Compatibility](https://eips.ethereum.org/EIPS/eip-8183#backwards-compatibility)
*   [Reference Implementation](https://eips.ethereum.org/EIPS/eip-8183#reference-implementation)
*   [Security Considerations](https://eips.ethereum.org/EIPS/eip-8183#security-considerations)
*   [Copyright](https://eips.ethereum.org/EIPS/eip-8183#copyright)

[](https://eips.ethereum.org/EIPS/eip-8183#abstract) Abstract
-------------------------------------------------------------
...
[](https://eips.ethereum.org/EIPS/eip-8183#motivation) Motivation
-----------------------------------------------------------------
...
[](https://eips.ethereum.org/EIPS/eip-8183#specification) Specification
-----------------------------------------------------------------------

The key words “MUST”, “MUST NOT”, “REQUIRED”, “SHALL”, “SHALL NOT”, “SHOULD”, “SHOULD NOT”, “RECOMMENDED”, “NOT RECOMMENDED”, “MAY”, and “OPTIONAL” in this document are to be interpreted as described in RFC 2119 and RFC 8174.

### [](https://eips.ethereum.org/EIPS/eip-8183#state-machine) State Machine
...
### [](https://eips.ethereum.org/EIPS/eip-8183#roles) Roles
...
### [](https://eips.ethereum.org/EIPS/eip-8183#job-data) Job Data
...
Payment SHALL use a single [ERC-20](https://eips.ethereum.org/EIPS/eip-20) token (global for the contract or specified at creation). Implementations MAY support a per-job token; the specification only requires one token per contract.

### [](https://eips.ethereum.org/EIPS/eip-8183#optional-provider-set-later) Optional provider (set later)
...
*   **fund(jobId, expectedBudget)** SHALL revert if `job.provider == address(0)` (provider MUST be set before funding) or if `job.budget != expectedBudget` (front-running protection).

### [](https://eips.ethereum.org/EIPS/eip-8183#core-functions) Core Functions

*   **createJob(provider, evaluator, expiredAt, description, hook?)** Called by client. Creates job in Open with `client = msg.sender`, `provider`, `evaluator`, `expiredAt`, `description`, and optional `hook` address. SHALL revert if `evaluator` is zero or `expiredAt` is not in the future. **Provider MAY be zero**; if so, client MUST call `setProvider` before `fund`. `hook` MAY be `address(0)` (no hook). Returns `jobId`.
*   **setProvider(jobId, provider, optParams?)** Called by client. SHALL revert if job is not Open, current `job.provider != address(0)`, or `provider == address(0)`. SHALL set `job.provider = provider`. `optParams` (bytes, OPTIONAL) is forwarded to the hook contract if set (see Hooks).
*   **setBudget(jobId, amount, optParams?)** Called by client or provider. Sets `job.budget = amount`. SHALL revert if job is not Open or caller is not client or provider. `optParams` forwarded to hook if set.
*   **fund(jobId, expectedBudget, optParams?)** Called by client. SHALL revert if job is not Open, caller is not client, budget is zero, **provider is not set** (`job.provider == address(0)`), or `job.budget != expectedBudget` (front-running protection). SHALL transfer `job.budget` of the payment token from client to the contract (escrow) and set status to Funded. `optParams` forwarded to hook if set.
*   **submit(jobId, deliverable, optParams?)** Called by provider only. SHALL revert if job is not Funded or caller is not the job’s provider. SHALL set status to Submitted. `deliverable` (`bytes32`) is a reference to submitted work (e.g. hash of off-chain deliverable, IPFS CID, attestation commitment). SHALL emit an event including `deliverable` (e.g. JobSubmitted). `optParams` forwarded to hook if set.
*   **complete(jobId, reason, optParams?)** Called by evaluator only. SHALL revert if job is not Submitted or caller is not the job’s evaluator. SHALL set status to Completed. SHALL transfer escrowed funds to provider (minus optional platform fee to a configurable treasury). `reason` MAY be `bytes32(0)` or an attestation hash (OPTIONAL). SHALL emit an event including `reason` if provided. `optParams` forwarded to hook if set.
*   **reject(jobId, reason, optParams?)** Called by **client when job is Open** or by **evaluator when job is Funded or Submitted**. SHALL revert if job is not Open, Funded, or Submitted, or caller is not the client (when Open) or the evaluator (when Funded or Submitted). SHALL set status to Rejected. If Funded or Submitted, SHALL refund escrow to client. `reason` OPTIONAL. SHALL emit an event including `reason` and the caller (rejector) if provided. `optParams` forwarded to hook if set.
*   **claimRefund(jobId)** Callable when job is Funded or Submitted and the job has expired (`block.timestamp >= expiredAt`). SHALL revert if job is not Funded or Submitted, or if the job has not yet expired. SHALL transfer full escrow to client and set status to Expired. MAY restrict caller (e.g. client only) or allow anyone; the specification RECOMMENDS allowing anyone to trigger refund after expiry.

### [](https://eips.ethereum.org/EIPS/eip-8183#attestation) Attestation
...
### [](https://eips.ethereum.org/EIPS/eip-8183#fees) Fees
...
### [](https://eips.ethereum.org/EIPS/eip-8183#hooks-optional) Hooks (OPTIONAL)
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#data-encoding) Data encoding
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#hook-behaviour) Hook behaviour
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#hook-security) Hook security
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#convenience-base-contract-non-normative) Convenience base contract (non-normative)
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#example-use-cases) Example use cases
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#example-1--fund-transfer-hook-two-phase-escrow) Example 1 — Fund Transfer Hook (two-phase escrow)
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#example-2--bidding-hook) Example 2 — Bidding Hook
...
### [](https://eips.ethereum.org/EIPS/eip-8183#events) Events
...
[](https://eips.ethereum.org/EIPS/eip-8183#rationale) Rationale
---------------------------------------------------------------
...
### [](https://eips.ethereum.org/EIPS/eip-8183#extensions-optional) Extensions (OPTIONAL)

The following extensions are OPTIONAL and do not modify the core protocol. Implementations MAY adopt them independently.

#### [](https://eips.ethereum.org/EIPS/eip-8183#reputation--attestation-interop-erc-8004) Reputation / Attestation Interop (ERC-8004)
...
#### [](https://eips.ethereum.org/EIPS/eip-8183#meta-transactions--facilitator-relay-erc-2771) Meta-Transactions / Facilitator Relay ([ERC-2771](https://eips.ethereum.org/EIPS/eip-2771))
...
[](https://eips.ethereum.org/EIPS/eip-8183#backwards-compatibility) Backwards Compatibility
-------------------------------------------------------------------------------------------

No backward compatibility issues found.

[](https://eips.ethereum.org/EIPS/eip-8183#reference-implementation) Reference Implementation
---------------------------------------------------------------------------------------------

TBD

[](https://eips.ethereum.org/EIPS/eip-8183#security-considerations) Security Considerations
-------------------------------------------------------------------------------------------

*   Evaluator is trusted for completion and rejection once the job is Submitted; a malicious evaluator can complete or reject arbitrarily. Use reputation (e.g. [ERC-8004](https://eips.ethereum.org/EIPS/eip-8004)) or staking for high-value jobs.
...
*   **Tokens:** Use SafeERC-20 or equivalent for [ERC-20](https://eips.ethereum.org/EIPS/eip-20).
...
[](https://eips.ethereum.org/EIPS/eip-8183#copyright) Copyright
---------------------------------------------------------------
...
Davide Crapis([@dcrapis](https://github.com/dcrapis)), Bryan Lim([@ai-virtual-b](https://github.com/ai-virtual-b)), Tay Weixiong([@twx-virtuals](https://github.com/twx-virtuals)), Chooi Zuhwa([@Zuhwa](https://github.com/Zuhwa)), "ERC-8183: Agentic Commerce [DRAFT]," _Ethereum Improvement Proposals_, no. 8183, February 2026. [Online serial]. Available: https://eips.ethereum.org/EIPS/eip-8183.
